resource "oci_core_instance" "TF_GPUInstance" {
        ...
        display_name                = "TF_${var.clusterName}_GPUnode${count.index}"
        shape                           = "${var.gpu_shape}"
        source_details {
                source_type = "image"
                source_id   = "${replace(var.gpu_shape, "GPU", "") != var.gpu_shape ? var.image_OracleLinux7_6_GPU[var.region] : var.image_OracleLinux7_6[var.region]}"
        }
